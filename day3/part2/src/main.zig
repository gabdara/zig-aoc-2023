...
fn run(reader: anytype, comptime line_len: usize) !usize {
    ...
    while (has_line) {
        ...
        for (line1, 0..) |c, pos| {
            ...
            if (symbol.isGear()) try symbols.append(symbol);
        }
    ...
    for (symbols.items) |symbol| {
        const items = symbol.numbers.items;
...
const Symbol = struct {
    ...
    pub fn isGear(self: *Symbol) bool {
        return self.value == '*' and self.numbers.items.len == 2;
    }
