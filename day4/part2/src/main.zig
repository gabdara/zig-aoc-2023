...
fn solvePuzzle(reader: anytype) !usize {
    ...
    for (cards.items, 1..) |*card, i| {
        const lucky_numbers = try NumberSet.intersection(u8, allocator, card.wining_numbers, card.user_numbers);
        if (lucky_numbers.len == 0 or i == cards.items.len) continue;
        card.references = cards.items[i..@min(i + lucky_numbers.len, cards.items.len)];
    }
